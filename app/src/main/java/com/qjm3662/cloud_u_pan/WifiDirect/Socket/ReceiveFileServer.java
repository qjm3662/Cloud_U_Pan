 ...
 * 服务器
 */
public class ReceiveFileServer extends ServerSocket {
    ...
    public static ReceiveFileServer instance;
    ...
    public ReceiveFileServer(ServerListener ss, boolean IsGroup) throws Exception {
        serverListener = ss;
        while (true) {
            try {
                ...
                fos = new FileOutputStream(new File(FileUtils.getWifiDirectPath() + fileName));
                byte[] sendBytes = new byte[1024];
                long transLen = 0;
                System.out.println("----开始接收文件<" + fileName + ">,文件大小为<" + fileLength + ">----");
                serverListener.FileInfoCallback(fileName, FileUtils.getWifiDirectPath() + fileName);
                ...
                serverListener.FileSuccessCallback();
            ...
            System.out.println("ReceiveFileServer Over !!!!----------------------------------------------");
        }
    }

    public static ReceiveFileServer getInstance(ServerListener serverListener, boolean IsGroup) {
        if (instance != null) {
            setServerListener(serverListener);
            return instance;
        } else {
            try {
                instance = new ReceiveFileServer(serverListener, IsGroup);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
